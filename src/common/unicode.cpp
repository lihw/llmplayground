...
static std::unordered_map<uint8_t, std::string> unicode_byte_to_utf8_map() {
    ...
    for (int ch = u'!'; ch <= u'~'; ++ch) {
        assert(0 <= ch && ch < 256);
        map[uint8_t(ch)] = unicode_cpt_to_utf8(uint32_t(ch));
    }
    for (int ch = u'¡'; ch <= u'¬'; ++ch) {
        assert(0 <= ch && ch < 256);
        map[uint8_t(ch)] = unicode_cpt_to_utf8(uint32_t(ch));
    ...
    for (int ch = 0; ch < 256; ++ch) {
        if (map.find(uint8_t(ch)) == map.end()) {
            map[uint8_t(ch)] = unicode_cpt_to_utf8(256 + uint32_t(n));
            ++n;
        }
    }
    return map;
...
static std::unordered_map<std::string, uint8_t> unicode_utf8_to_byte_map() {
    ...
    for (int ch = u'!'; ch <= u'~'; ++ch) {
        assert(0 <= ch && ch < 256);
        map[unicode_cpt_to_utf8(uint32_t(ch))] = uint8_t(ch);
    }
    for (int ch = u'¡'; ch <= u'¬'; ++ch) {
        assert(0 <= ch && ch < 256);
        map[unicode_cpt_to_utf8(uint32_t(ch))] = uint8_t(ch);
    ...
    auto n = 0;
    for (int ch = 0; ch < 256; ++ch) {
        if (map.find(unicode_cpt_to_utf8(uint32_t(ch))) == map.end()) {
            map[unicode_cpt_to_utf8(256 + uint32_t(n))] = uint8_t(ch);
            ++n;
        }
    }
    return map;
